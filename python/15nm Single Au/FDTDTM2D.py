...
# import numpy as np
# import multiprocessing as mp
# import FDTDTM2D as TM
# l = TM.FDTD2D(1, 4.46-2.53j, 532E-9, 15E-9, 2E-8, 0, 51, np.arange(50,350))
# l.DefinePML()
# l.DefineSphere()
...
class FDTD2D:
    def __init__(self, e_m, e_p, wl, a, x, y, srcx, srcy):
        ...
        self.e = e0 * self.e_m * np.ones((Ny, Nx), dtype=complex)
        self.sigma_e = self.sigma_e_m * np.ones((Ny, Nx), dtype=complex)
        self.mu = m0 * self.mu_m * np.ones((Ny, Nx), dtype=complex)
    ...
    def DefineSphere(self):
        ...
        for (cx, cy) in [(self.x, self.y)]:
            ...
            self.e = self.SphereParams(self.e, x_logic, y_logic, a_mesh, e0 * self.e_p)
            # self.sigma_e = self.SphereParams(self.sigma_e, x_logic, y_logic, a_mesh, -e0 * np.imag(self.e_p) * self.w)
            self.mu = self.SphereParams(self.mu, x_logic, y_logic, a_mesh, m0 * self.mu_p)
    ...
    def RunFDTD(self):
        ...
        t = 0
        FigureParam = self.FigureOpen(self.Hz, self.Hzb, True, True, True)
        ...
        while True:
            ...
            self.PlaneWave(self.Hz, self.Hzb, self.srcx, self.srcy, t)
    ...
    def PointSource(self, E, Eb, x, y, t):

        E[y, x] = np.sin(self.w*dt*t)
        Eb[y, x] = np.sin(self.w*dt*t)

    def PlaneWave(self, E, Eb, x, y, t):
        if 2*self.w*dt*t<np.pi/100:
            E[y, x] = 35*np.sin(2*self.w*dt*t)
            Eb[y, x] = 35*np.sin(2*self.w*dt*t)
        # else:
        #     E[y, x] = 0
        #     Eb[y, x] = 0

    def FigureOpen(self, E, Eb, f1=True, f2=True, f3=True):
    ...
    def FigureSettings(self, E, fig, ax, cx, ftitle):
        ...
        ax.imshow(self.e == e0 * self.e_p, alpha=0.2, cmap='Purples', origin='lower', aspect=1, vmin=0, vmax=1,
                  extent=[-E.shape[1] * dx * NUNIT / 2, E.shape[1] * dx * NUNIT / 2,
                          -E.shape[0] * dy * NUNIT / 2, E.shape[0] * dy * NUNIT / 2])
