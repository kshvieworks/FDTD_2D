...
[Lx, Ly] = [3E-7, 3E-7] # [m]
[Nx, Ny] = [301, 301] # Number of Mesh Point
...
n = nAu
k = kAu
material_e = np.power(n, 2) - np.power(k, 2) - 1j*2*n*k
...
e_sigma[((x_grid[np.newaxis, :] - (e.shape[0]/2 + cx1/dx))**2) + ((y_grid[:, np.newaxis] - (e.shape[1]/2 + cy1/dy))**2) < (cr/dx)**2] = np.imag(material_e)*(2*np.pi*v)
...
cx1 = ax1.imshow(np.abs(Ez), cmap='bwr', origin='lower', aspect=1, vmin=-1, vmax=1, alpha=1,
...
ax1.imshow(e < e0, alpha = 0.2, cmap='Purples', origin='lower', aspect=1, vmin=-1, vmax=1,
           extent=[-Ez.shape[1] * dx * nunit / 2, Ez.shape[1] * dx * nunit / 2,
                   -Ez.shape[0] * dy * nunit / 2, Ez.shape[0] * dy * nunit / 2])

ax1.imshow(e_sigma > 0, alpha=0.1, cmap='Greys', origin='lower', aspect=1, vmin=-1, vmax=1,
           extent=[-Ez.shape[1] * dx * nunit / 2, Ez.shape[1] * dx * nunit / 2,
                   -Ez.shape[0] * dy * nunit / 2, Ez.shape[0] * dy * nunit / 2])
...
cx3 = ax3.imshow(np.abs(Ez - Ebz), cmap='bwr', origin='lower', aspect=1, vmin=-1, vmax=1,
               extent=[-Ez.shape[1] * dx * nunit / 2, Ez.shape[1] * dx * nunit / 2,
                       -Ez.shape[0] * dy * nunit / 2, Ez.shape[0] * dy * nunit / 2])
...
ax3.imshow(e < e0, alpha = 0.2, cmap='Purples', origin='lower', aspect=1, vmin=-1, vmax=1,
           extent=[-Ez.shape[1] * dx * nunit / 2, Ez.shape[1] * dx * nunit / 2,
                   -Ez.shape[0] * dy * nunit / 2, Ez.shape[0] * dy * nunit / 2])

ax3.imshow(e_sigma > 0, alpha=0.1, cmap='Greys', origin='lower', aspect=1, vmin=-1, vmax=1,
           extent=[-Ez.shape[1] * dx * nunit / 2, Ez.shape[1] * dx * nunit / 2,
                   -Ez.shape[0] * dy * nunit / 2, Ez.shape[0] * dy * nunit / 2])
...
# Define Source
#     if t < 1:
#         Ebz[int(Ebz.shape[0] / 2)::int(3*Ebz.shape[1] / 4), int(Ebz.shape[1] / 4)] = 1
#         Ez[int(Ez.shape[0] / 4):int(3*Ebz.shape[1] / 4), int(Ez.shape[1] / 4)] = 1
    if t < 30:
        Ebz[int(Ebz.shape[0]/2), int(Ebz.shape[1]/4)] = np.sin(2*np.pi*v*dt*t)
        Ez[int(Ez.shape[0] / 2), int(Ez.shape[1] / 4)] = np.sin(2 * np.pi * v * dt * t)

    # Ebz[int(Ebz.shape[0]/4):int(3*Ebz.shape[0]/4), int(Ebz.shape[1]/4)] = np.sin(2*np.pi*v*dt*t)
    # Ez[int(Ez.shape[0]/4):int(3*Ebz.shape[0]/4), int(Ez.shape[1]/4)] = np.sin(2 * np.pi * v * dt * t)


    t = t + 1

    cx1.set_data(np.abs(Ez))
    cx2.set_data(np.real(Ebz))
    cx3.set_data(np.abs(Ez - Ebz))
