...
def animate_func4(k):
    im4.set_data(np.abs(Render_Ez[k, :, :]))
    t4.set_text(f"Time: {round(dt * k * 1E15, 3)} [1E-15 s]")

def animate_func5(k):
    im5.set_data(np.abs(Render_Ebz[k, :, :]))
    t5.set_text(f"Time: {round(dt * k * 1E15, 3)} [1E-15 s]")

def animate_func6(k):
    im6.set_data(np.abs(Render_Ez[k, :, :] - Render_Ebz[k, :, :]))
...
def handle_exit():
    ...
    ax4 = fig4.subplots()
    im4 = ax4.imshow(np.abs(Render_Ez[0, :, :]), cmap='bwr', origin='lower', aspect=1, vmin=-1/4, vmax=1/4, alpha=1,
    ...
    ax5 = fig5.subplots()
    im5 = ax5.imshow(np.abs(Render_Ez[0, :, :]), cmap='bwr', origin='lower', aspect=1, vmin=-1/4, vmax=1/4, alpha=1,
               extent=[-Render_Ez.shape[2] * dx * nunit / 2, Render_Ez.shape[2] * dx * nunit / 2,
                       -Render_Ez.shape[1] * dy * nunit / 2, Render_Ez.shape[1] * dy * nunit / 2])
    ...
    ax6 = fig6.subplots()
    im6 = ax6.imshow(np.abs(Render_Ez[0, :, :]), cmap='bwr', origin='lower', aspect=1, vmin=-1/4, vmax=1/4, alpha=1,
               extent=[-Render_Ez.shape[2] * dx * nunit / 2, Render_Ez.shape[2] * dx * nunit / 2,
                       -Render_Ez.shape[1] * dy * nunit / 2, Render_Ez.shape[1] * dy * nunit / 2])
...
e[((x_grid[np.newaxis, :] - (e.shape[0]/2 + cx2/dx))**2) + ((y_grid[:, np.newaxis] - (e.shape[1]/2 + cy2/dy))**2) < (cr/dx)**2] = material_e * e0
...
cx1 = ax1.imshow(np.abs(Ez), cmap='bwr', origin='lower', aspect=1, vmin=-1/4, vmax=1/4, alpha=1,
...
cx2 = ax2.imshow(np.abs(Ebz), cmap='bwr', origin='lower', aspect=1, vmin=-1/4, vmax=1/4,
               extent=[-Ez.shape[1] * dx * nunit / 2, Ez.shape[1] * dx * nunit / 2,
                       -Ez.shape[0] * dy * nunit / 2, Ez.shape[0] * dy * nunit / 2])
...
cx3 = ax3.imshow(np.abs(Ez - Ebz), cmap='bwr', origin='lower', aspect=1, vmin=-1/4, vmax=1/4,
               extent=[-Ez.shape[1] * dx * nunit / 2, Ez.shape[1] * dx * nunit / 2,
                       -Ez.shape[0] * dy * nunit / 2, Ez.shape[0] * dy * nunit / 2])
